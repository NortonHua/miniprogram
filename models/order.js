...
class Order {
    ...
    static async getPaidCount() {
        const orderPage = await Http.request({
            url: `/wx-order/pageByStatus/${OrderStatus.PAID}`,
            data:{
                start:0,
                count:1
            }
    ...
    static async getUnpaidCount() {
        const orderPage = await Http.request({
            url: `/wx-order/pageByStatus/${OrderStatus.UNPAID}`,
            data:{
                start:0,
                count:1
            }
    ...
    static async getDeliveredCount() {
        const orderPage = await Http.request({
            url: `/wx-order/pageByStatus/${OrderStatus.DELIVERED}`,
            data: {
                start:0,
                count:1
            }
    ...
    static getPagingByStatus(status) {
        return new Paging({
            url:`/wx-order/pageByStatus/${status}`
        })
        // return Http.request({
        // })
    }
